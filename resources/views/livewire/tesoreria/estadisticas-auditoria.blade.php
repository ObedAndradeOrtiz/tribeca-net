<div>
    ...
    <div class="stats-page">
        ...
        <div class="stats-kpi-grid" wire:loading.remove wire:target="anio,mes,cargarEstadisticas">
            ...
            <div class="stats-kpi dark">
                ...
                <strong>Bs {{ number_format($saldoMes, 2) }}</strong>
                <small>Ingresos menos egresos</small>
            </div>

            <div class="stats-kpi warning">
                <span>Deuda total</span>
                <strong>Bs {{ number_format($totalDeuda, 2) }}</strong>
                <small>{{ $cantidadDeudores }} departamentos deudores</small>
            </div>

            <div class="stats-kpi info">
                <span>Ingresos pendientes</span>
                <strong>Bs {{ number_format($montoIngresosPendientes, 2) }}</strong>
                <small>{{ $ingresosPendientes }} movimientos pendientes</small>
            </div>

            <div class="stats-kpi orange">
                <span>Egresos sin comprobante</span>
                <strong>Bs {{ number_format($montoEgresosSinComprobante, 2) }}</strong>
                <small>{{ $egresosSinComprobante }} registros sin archivo</small>
            </div>
            ...
            <div class="stats-card wide">
                <div class="stats-card-head">
                    <div>
                        ...
                        <p>Comparativo mensual de la gestión {{ $anio }}.</p>
                    </div>
                </div>
            ...
            <div class="stats-card">
                <div class="stats-card-head">
                    <div>
                        ...
                        <p>{{ $this->nombreMes((int) $mes) }} {{ $anio }}</p>
                    </div>
                </div>
                <div class="chart-box">
        ...
        <div class="stats-bottom-grid" wire:loading.remove wire:target="anio,mes,cargarEstadisticas">
            ...
            <div class="stats-card">
                <div class="stats-card-head">
                    <div>
                        <h4>Resumen anual</h4>
                        <p>Totales generales de la gestión {{ $anio }}.</p>
                    </div>
                </div>
                ...
                <div class="stats-number-list">
                    <div>
                        ...
                        <strong class="green">Bs {{ number_format($totalIngresosAnio, 2) }}</strong>
                    </div>

                    <div>
                        <span>Egresos gestión</span>
                        <strong class="red">Bs {{ number_format($totalEgresosAnio, 2) }}</strong>
                    </div>

                    <div>
                        <span>Saldo gestión</span>
                        <strong class="{{ $saldoAnio >= 0 ? 'green' : 'red' }}">
                            Bs {{ number_format($saldoAnio, 2) }}
                        </strong>
                    </div>
                </div>
            </div>
            ...
            <div class="stats-card">
                ...
                <div class="stats-number-list">
                    <div>
                        ...
                        <strong class="green">Bs {{ number_format($totalIngresosGeneral, 2) }}</strong>
                    </div>

                    <div>
                        <span>Egresos históricos</span>
                        <strong class="red">Bs {{ number_format($totalEgresosGeneral, 2) }}</strong>
                    </div>

                    <div>
                        <span>Saldo histórico</span>
                        <strong class="{{ $saldoGeneral >= 0 ? 'green' : 'red' }}">
            ...
            <div class="stats-card">
                ...
                <div class="stats-table-wrap">
                    <table class="stats-table">
                        ...
                        <tbody>
                            @forelse ($departamentosIrregulares as $item)
                                <tr>
                                    <td><strong>{{ $item['departamento'] }}</strong></td>
                                    <td class="text-end red">
                                        Bs {{ number_format($item['deuda'], 2) }}
                                    </td>
                                    <td>{{ $item['parciales'] }}</td>
                                    <td>{{ $item['pendientes'] }}</td>
                                    <td>{{ $item['no_cobrables'] }}</td>
                                </tr>
                            @empty
                                <tr>
    ...
    <script>
        ...
        document.addEventListener('livewire:load', function() {
            renderEstadisticas({
                ingresosMensuales: @json($chartIngresosMensuales),
                egresosMensuales: @json($chartEgresosMensuales),
                saldoMensual: @json($chartSaldoMensual),
                comparativoMes: @json($chartComparativoMes),
                ingresosEspeciales: @json($chartIngresosEspeciales),
                estadoDepartamentos: @json($chartEstadoDepartamentos),
                topDeudores: @json($chartTopDeudores),
                irregularidades: @json($chartIrregularidades),
            });
