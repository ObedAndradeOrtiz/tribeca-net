<div>
    ...
    <x-modal wire:model.defer="rellamar">
        <div class="px-6 py-4">
    ...
    <x-modal wire:model.defer="openArea4">
        <div class="px-6 py-4">
            <div class="text-lg font-medium text-gray-900">
                Confirmar cita
            </div>
    ...
    <x-modal wire:model.defer="vermitratamiento">
        <div class="px-6 py-4">
            <div class="text-lg font-medium text-gray-900">
                                ...
                                <table id="user-list-table" class="table table-striped" role="grid"
                                    ...
                                    <tbody>
                                        ...
                                        {{-- @php
                                            ...
                                            <td>{{ $total_pagados }}</td>
                                            @php
                                                $fechadepago = 'No existe';
                                                $fecha_formateada = 'No existe';
                                                $cantidad = DB::table('pagos')
                                                    ->where('iduser', $lista->id)
                                                    ->where('estado', 'Activo')
                                                    ->count();
                                                if ($cantidad > 0) {
                                                    $paguitos = DB::table('pagos')
                                                        ->where('iduser', $lista->id)
                                                        ->where('estado', 'Activo')
                                                        ->first();
                                                    $fechadepago = $paguitos->fechainicio;
                                                    setlocale(LC_TIME, 'es_ES'); // Establece la configuración regional para mostrar la fecha en español
                                                    $fecha = $fechadepago; // la cadena de fecha en formato oro

                                                    $marca_tiempo = strtotime($fecha); // convierte la cadena en una marca de tiempo Unix

                                                    $fecha_formateada = strftime('%A %e de %B del %Y', $marca_tiempo);
                                                }
                                            @endphp
