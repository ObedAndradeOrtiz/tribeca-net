...
<body>
        ...
        <div class="cover-box">
            ...
            <table class="cover-info">
                <tr>
                    ...
                    <td class="value">{{ $auditor['nombre'] ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Carnet de identidad</td>
                    <td class="value">{{ $auditor['carnet'] ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Cargo / Rol</td>
                    <td class="value">{{ $auditor['cargo'] ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Fecha de realización</td>
                    <td class="value">
                        @if (!empty($auditor['fecha_realizacion']))
                            {{ \Carbon\Carbon::parse($auditor['fecha_realizacion'])->format('d/m/Y') }}
                        @else
                            -
                        @endif
                    </td>
                ...
                <tr>
                    ...
                    <td class="value">
                        @if (!empty($auditor['trabajo_inicio']))
                            {{ \Carbon\Carbon::parse($auditor['trabajo_inicio'])->format('d/m/Y') }}
                        @else
                            -
                        @endif
                    </td>
                ...
                <tr>
                    ...
                    <td class="value">
                        @if (!empty($auditor['trabajo_fin']))
                            {{ \Carbon\Carbon::parse($auditor['trabajo_fin'])->format('d/m/Y') }}
                        @else
                            -
                        @endif
                    </td>
    ...
    <div class="section">
        ...
        <table class="summary-list">
            ...
            <tr>
                ...
                <td class="value">{{ $textoRango }}</td>
            </tr>
            <tr>
                <td class="label">Rango exacto de fechas</td>
                <td class="value">
                    {{ \Carbon\Carbon::parse($rango['fecha_inicio'])->format('d/m/Y') }}
                    al
                    {{ \Carbon\Carbon::parse($rango['fecha_fin'])->format('d/m/Y') }}
                </td>
            </tr>
            <tr>
    ...
    <div class="section">
        ...
        <p>
            ...
            considerando como fecha inicial de auditoría el mes de <strong>{{ $rango['texto_inicio'] }}</strong>
            y como fecha final el mes de <strong>{{ $rango['texto_fin'] }}</strong>.
        </p>
    ...
    <div class="section avoid-break">
        ...
        <table class="kpi-table">
            <tr>
                <td class="kpi">
                    ...
                    <span class="kpi-value green">Bs {{ number_format($totalIngresos, 2) }}</span>
                </td>

                <td class="kpi">
                    <span class="kpi-label">Total egresos auditados</span>
                    <span class="kpi-value red">Bs {{ number_format($totalEgresos, 2) }}</span>
                </td>

                <td class="kpi">
                    <span class="kpi-label">Saldo neto del periodo</span>
                    <span class="kpi-value {{ $saldoFinal >= 0 ? 'green' : 'red' }}">
                ...
                <td class="kpi">
                    ...
                    <span class="kpi-value dark">{{ $totalDepartamentosRevisados }}</span>
                </td>
            </tr>
        </table>
        ...
        <table class="summary-list">
            <tr>
                ...
                <td class="value red">{{ $totalDeptosIrregulares }}</td>
            </tr>
            <tr>
                <td class="label">Departamentos sin observaciones relevantes</td>
                <td class="value green">{{ $totalDeptosSinIrregularidad }}</td>
            </tr>
            <tr>
                <td class="label">Ingresos destinados o asociados a gestión anterior</td>
                <td class="value orange">Bs {{ number_format($totalGestionAnterior, 2) }}</td>
            </tr>
            <tr>
                <td class="label">Ingresos por alquiler de salón</td>
                <td class="value blue">Bs {{ number_format($totalSalon, 2) }}</td>
            </tr>
            <tr>
                <td class="label">Otros ingresos identificados</td>
                <td class="value dark">Bs {{ number_format($totalOtros, 2) }}</td>
            </tr>
        </table>
    </div>
    ...
    <div class="section">
        ...
        <table class="data">
            ...
            <tbody>
                @forelse($ingresosSinComprobante as $item)
                    <tr>
                        <td>
                            @if (!empty($item->fecha))
                        ...
                        <td>{{ $item->numero_comprobante ?? 'Sin comprobante' }}</td>
                        <td>{{ $item->detalle ?? '-' }}</td>
                        <td class="text-right">Bs {{ number_format((float) ($item->monto ?? 0), 2) }}</td>
                    </tr>
                @empty
                    <tr>
        ...
        <table class="data">
            ...
            <tbody>
                @forelse($egresosSinComprobante as $item)
                    <tr>
                        <td>
                            @php
                            ...
                            @if ($fechaEgreso)
                                {{ \Carbon\Carbon::parse($fechaEgreso)->format('d/m/Y') }}
                            @else
                                -
                            @endif
                        </td>
                        <td>{{ $item->empresa ?? 'Gasto registrado' }}</td>
                       
                        <td>{{ $item->estado ?? '-' }}</td>
                        <td class="text-right">Bs {{ number_format((float) ($item->cantidad ?? 0), 2) }}</td>
    ...
    <div class="section">
        ...
        <table class="data">
            ...
            <tbody>
                @forelse($departamentosResumen as $item)
                    <tr>
                        <td><strong>{{ $item['departamento'] }}</strong></td>

                        <td class="text-right">
                            Bs {{ number_format((float) ($item['total_deuda'] ?? 0), 2) }}
                        </td>

                        <td class="text-center">{{ $item['meses_con_deuda'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['meses_parciales'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['meses_pendientes'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['meses_no_cobrables'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['pagos_fuera_fecha'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['pagos_con_descuento'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['descuentos_observados'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['pagos_anticipados_fuera_mes'] ?? 0 }}</td>

                        <td>
                            @if (!empty($item['tiene_irregularidad']))
    ...
    <div class="section">
        ...
        <table class="data">
            ...
            <tbody>
                @forelse($departamentosConIrregularidad as $item)
                    <tr>
                        <td><strong>{{ $item['departamento'] }}</strong></td>
                        <td class="text-right">Bs {{ number_format((float) ($item['total_deuda'] ?? 0), 2) }}</td>
                        <td class="text-center">{{ $item['meses_con_deuda'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['pagos_fuera_fecha'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['descuentos_observados'] ?? 0 }}</td>
                        <td class="text-center">{{ $item['pagos_anticipados_fuera_mes'] ?? 0 }}</td>
                        <td>
                            @if (($item['total_deuda'] ?? 0) > 0)
                                Presenta saldo pendiente.
                            @elseif(($item['pagos_fuera_fecha'] ?? 0) > 0)
                                Tiene pagos fuera de fecha límite.
                            @elseif(($item['descuentos_observados'] ?? 0) > 0)
                                Requiere revisión de descuento.
                            @elseif(($item['pagos_anticipados_fuera_mes'] ?? 0) > 0)
    ...
    {{-- IRREGULARIDADES DETALLADAS SI EXISTEN --}}
                ...
                <tbody>
                    @foreach ($irregularidades as $item)
                        <tr>
                            ...
                            <td>{{ $item['detalle'] ?? '-' }}</td>
                            <td class="text-right">
                                Bs {{ number_format((float) ($item['monto'] ?? 0), 2) }}
                            </td>
                            <td>
    ...
    {{-- DEPARTAMENTOS SIN IRREGULARIDAD --}}
    <div class="section">
        ...
        <table class="data">
            ...
            <tbody>
                @forelse($departamentosSinIrregularidad as $item)
                    <tr>
                        <td><strong>{{ $item['departamento'] }}</strong></td>
                        <td class="text-right">Bs {{ number_format((float) ($item['total_deuda'] ?? 0), 2) }}</td>
                        <td class="text-center">{{ $item['meses_con_deuda'] ?? 0 }}</td>
    ...
    <div class="section">
        ...
        <table class="summary-list">
            <tr>
                ...
                <td class="value">{{ $totalDepartamentosRevisados }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="value orange">{{ $ingresosSinComprobante->count() }}</td>
            </tr>
            <tr>
                <td class="label">Cantidad de egresos sin comprobante</td>
                <td class="value orange">{{ $egresosSinComprobante->count() }}</td>
            </tr>
        </table>
    ...
    <div class="section">
        ...
        <div class="conclusion-box">
            <p>
                ...
                <strong>{{ $rango['texto_inicio'] }}</strong> y <strong>{{ $rango['texto_fin'] }}</strong>,
            ...
            <p>
                ...
                <strong>Bs {{ number_format($totalIngresos, 2) }}</strong>,
                egresos auditados por
                <strong>Bs {{ number_format($totalEgresos, 2) }}</strong>,
                y un saldo neto del periodo de
                <strong>Bs {{ number_format($saldoFinal, 2) }}</strong>.
            </p>

            <p>
                Se revisaron <strong>{{ $totalDepartamentosRevisados }}</strong> departamentos,
                de los cuales <strong>{{ $totalDeptosIrregulares }}</strong> presentan deuda, irregularidad
                u observación administrativa, mientras que
                <strong>{{ $totalDeptosSinIrregularidad }}</strong> no presentan observaciones relevantes
            ...
            <tr>
                <td>
                    <div class="signature-line">
                        {{ $auditor['nombre'] ?? 'Auditor / Responsable' }}
                    </div>
                    <div class="signature-sub">
                        C.I. {{ $auditor['carnet'] ?? '-' }}<br>
                        {{ $auditor['cargo'] ?? 'Responsable de auditoría' }}
                    </div>
                </td>
